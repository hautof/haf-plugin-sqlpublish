...
class DBWebCaseIds(object):
    def __init__(self, id, case_id, case_sub_id, case_name, case_web_name):
        self.id = id if id is not None else ''
        self.case_id = case_id if case_id is not None else ''
        self.case_sub_id = case_sub_id if case_sub_id is not None else ''
        self.case_name = case_name if case_name is not None else ''
        self.case_web_name = case_web_name if case_web_name is not None else ''


class DBWebCaseCaps(object):
    def __init__(self, id, platform_name, platform_version, start_url):
        self.id = id if id is not None else ''
        ...
        self.start_url = start_url if start_url is not None else ''


class DBWebCaseStage(object):
    def __init__(self, id, stage_id, name, operation, show_try, time_sleep, info, result_id, web_case_id, run_count):
        self.id = id if id is not None else ''
        ...
        self.web_case_id = web_case_id if web_case_id is not None else ''
        self.run_count = run_count if run_count is not None else ''


class DBWebCaseStageResult(object):
...
class DBWebCaseStagePath(object):
...
class DBWebCaseSqlinfoChecklist(object):
    def __init__(self, id, sql_response):
        self.id = id if id is not None else ''
        self.sql_response = sql_response if sql_response is not None else ''


class DBWebCaseSqlinfoScript(object):
    def __init__(self, id, sql_response):
        self.id = id if id is not None else ''
        self.sql_response = sql_response if sql_response is not None else ''


class DBWebCaseSqlinfoConfig(object):
...
class SQLWebublish(object):
    def __init__(self):
        pass

    def insert_web_detail(self, db_detail: DBWebDetail):
        ...
        sql_sc = [
            f"""insert into web_detail ( case_name, result_check_response, result_check_sql_response, run_error, result, begin_time, end_time, log_dir, runner)
    ...
    def insert_web_case(self, db_case: DBWebCase):
        if isinstance(db_case.dependent, list):
            dependent = ";".join([str(x) for x in db_case.dependent])
        else:
            dependent = str(db_case.dependent)
        sql_sc = [
            f"""insert into web_case (ids_id, `run`, dependent, bench_name, expect_id, sqlinfo_id, `type`, suite_id, detail_id, caps_id) 
    ...
    def insert_web_caseexpect(self, db_case_expect: DBWebCaseExpect):
        ...
        sql_sc = [
            f"""insert into web_case_expect (response_id, sql_check_func, sql_response_result)
                    values ('{db_case_expect.response_id}', '{check_func}', '{sql_response_result}');""",
            f"SELECT @@IDENTITY AS Id"]
        return sql_sc

    def insert_web_case_ids(self, db_caseid: DBWebCaseIds):
        sql_sc = [
            f"""insert into web_case_ids (case_id, case_sub_id, case_name, case_web_name) 
                    values ('{db_caseid.case_id}', '{db_caseid.case_sub_id}', '{db_caseid.case_name}', '{db_caseid.case_web_name}');  """,
            f"SELECT @@IDENTITY AS Id"]
        return sql_sc

    def insert_web_case_caps(self, db_caps: DBWebCaseCaps):
        sql_sc = [
            f"""insert into web_case_caps ( platform_name, platform_version, start_url) 
                    values ('{db_caps.platform_name}', '{db_caps.platform_name}', '{db_caps.start_url}');  """,
            f"SELECT @@IDENTITY AS Id"]
        return sql_sc

    def insert_web_case_stage_result(self, db_case_stage_result: DBWebCaseStageResult):
        sql_sc = [
            f"""insert into web_case_stage_result (result, `exception`) 
                    values ('{db_case_stage_result.result}', '{db_case_stage_result.exception}');  """,
            f"SELECT @@IDENTITY AS Id"]
        return sql_sc

    def insert_web_case_stage(self, db_case_stage: DBWebCaseStage):
        ...
        info = x 
        sql_sc = [
            f"""insert into web_case_stage (stage_id, name, operation, show_try, time_sleep, info, result_id, web_case_id, run_count) 
                    values ('{db_case_stage.stage_id}', '{db_case_stage.name}', '{db_case_stage.operation}', '{db_case_stage.show_try}', '{db_case_stage.time_sleep}', '{info}', '{db_case_stage.result_id}', '{db_case_stage.web_case_id}',  '{db_case_stage.run_count}');  """,
            f"SELECT @@IDENTITY AS Id"]
        return sql_sc

    def insert_web_case_stage_path(self, db_case_stage_path: DBWebCaseStagePath):
        path = db_case_stage_path.find_value
        if isinstance(path, str):
            path = path.replace("'", "\\'").replace('"', '\\"')
        sql_sc = [
            f"""insert into web_case_stage_path (stage_id, find_type, find_value) 
                    values ('{db_case_stage_path.stage_id}', '{db_case_stage_path.find_type}', '{path}');""",
            f"SELECT @@IDENTITY AS Id"]
        return sql_sc

    def insert_web_case_sqlinfo(self, db_casesqlinfo: DBWebCaseSqlinfo):
        sql_sc = [
            f"""insert into web_case_sqlinfo (scripts_id, config_id, check_list_id)
                    values ('{db_casesqlinfo.scripts_id}', '{db_casesqlinfo.config_id}', '{db_casesqlinfo.check_list_id}'); """,
            f"SELECT @@IDENTITY AS Id"]
        return sql_sc

    def insert_web_case_sqlinfochecklist(self, db_casesqlinfo_checklist: DBWebCaseSqlinfoChecklist):
        ...
        sql_sc = [
            f"""insert into web_case_sqlinfo_checklist (sql_response) values ('{sql_response_checklist}');""",
            f"SELECT @@IDENTITY AS Id "]
        return sql_sc

    def insert_web_case_sqlinfoscript(self, db_casesqlinfo_script: DBWebCaseSqlinfoScript):
        ...
        sql_sc = [
            f"""insert into web_case_sqlinfo_script (sql_response) values ('{sql_response_script}');""",
            f" SELECT @@IDENTITY AS Id "]
        return sql_sc

    def insert_web_case_sqlinfoconfig(self, db_casesqlinfo_config: DBWebCaseSqlinfoConfig):
        sql_sc = [
            f"""insert into web_case_sqlinfo_config (`host`, `port`, `type`, `username`, `password`)
